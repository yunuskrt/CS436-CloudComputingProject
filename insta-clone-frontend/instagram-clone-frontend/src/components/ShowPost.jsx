...
function ShowPost(props) {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      await axiosJWT.post("https://api-nodejs-y7qorp3gha-ey.a.run.app/api/comment/", submitData, {
  ...
  useEffect(() => {
    const fetchComment = async () => {
      const res = await axios.get(
        `https://api-nodejs-y7qorp3gha-ey.a.run.app/api/comment/${props.post._id}`
      );

      res.data.comments.forEach(async (comment) => {
        const res = await axios.get(
          `https://api-nodejs-y7qorp3gha-ey.a.run.app/api/user/${comment.user}`
        );
